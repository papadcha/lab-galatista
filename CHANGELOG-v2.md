...
Ο χρήστης ανέφερε ότι μια εγκατεστημένη v2.4.0 δεν έδειχνε ποτέ το update
banner για το v2.4.1, ακόμα και μετά από πλήρες restart. Ρίζα: το
background auto-download του installer (νέο στο v2.4.0) γινόταν με `await`
**πριν** σταλεί το `update-available` στο renderer, χωρίς κανένα timeout —
μια κολλημένη/αργή σύνδεση (π.χ. VPN) κρατούσε ολόκληρο το
`checkForUpdates()` σε εκκρεμότητα επ' άπειρον, χωρίς κανένα σφάλμα να το
προδίδει, άρα το banner απλά δεν εμφανιζόταν ποτέ.

Fix, δύο μέρη:
- **Timeout σε κάθε net.request** (modules/update-check.js) — τόσο στο
  fetch του manifest (15s) όσο και στη λήψη του installer (20s idle
  timeout, resets σε κάθε chunk ώστε μια απλά αργή αλλά ενεργή λήψη ~100MB
  να μην κοπεί άδικα). Μηχανισμός: `setTimeout` + `request.abort()`,
  επαληθεύτηκε ζωντανά ότι πραγματικά κόβει μια κολλημένη σύνδεση (test
  προς μη-δρομολογήσιμη IP, timeout σε ~3s όπως αναμενόταν αντί για hang).
- **Το banner πλέον εμφανίζεται αμέσως**, χωρίς να περιμένει τη λήψη —
  ξεκινά ως "⬇ Λήψη" (fallback: άνοιγμα browser). Αν η background λήψη
  ολοκληρωθεί μετά, νέο IPC event `update-ready` αναβαθμίζει το ήδη ορατό
  κουμπί σε "⚙ Εγκατάσταση" χωρίς να ξαναχτίζει το banner.

Επαληθεύτηκε ζωντανά (Playwright, προσομοίωση IPC events): το banner
εμφανίζεται αμέσως με "⬇ Λήψη", αναβαθμίζεται σε "⚙ Εγκατάσταση" όταν
φτάσει το `update-ready`, και ο timeout μηχανισμός σταματάει πραγματικά
μια κολλημένη σύνδεση αντί να κρεμάσει.

**Αρχεία:**
- `modules/update-check.js`, `preload.cjs`, `src/main-app.js`
