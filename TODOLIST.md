...
Τρία ξεχωριστά items που ο χρήστης είχε αποφασίσει να ξεκινήσουν ΜΟΝΟ
αφού ολοκληρωθούν και τα δύο μεγάλα updates παραπάνω (ESM redesign +
i18n) — και τα δύο ΟΛΟΚΛΗΡΩΘΗΚΑΝ (merge v2.0.0 μόλις έγινε, i18n
per-page + backend PDF + reports.js preview όλα done). Ελεύθερα να
ξεκινήσουν πλέον, όχι ακόμα ζητημένα ρητά.
...
- [ ] Μετονομασία εφαρμογής σε "ΔAiγμα LiMS" (άλλαξε από "Aggregate
      Inspector Ai", 2026-07-08) + αλλαγή icon — το i18n που το
      μπλόκαρε ολοκληρώθηκε, το v2.0.0 merge επίσης έγινε. Χρειάζεται
      ΝΕΟ λογότυπο/wordmark (το παλιό ήταν φτιαγμένο για το προηγούμενο
      όνομα, δεν επαναχρησιμοποιείται).
