...
- [ ] Ώρα καταχώρησης + χρονόμετρο δοκιμών — προτάθηκε 2026-08-04. Δύο
      σχετικές ιδέες: (α) εμφάνιση ώρας (όχι μόνο ημερομηνίας) στην
      καταχώρηση, (β) χρονόμετρο για τις δοκιμές. Καμία σχεδίαση δεν
      έχει γίνει ακόμα — μόνο καταγραφή της ιδέας.

- [ ] Καταγραφή καιρού ανά δείγμα — ο καιρός παίζει ρόλο στα αποτελέσματα
      δειγματοληψίας. Το FreeMeteo.gr (προτίμηση για ακρίβεια πρόγνωσης)
      **δεν έχει δημόσιο API** (επιβεβαιώθηκε 2026-08-04) — μόνο site
      πρόγνωσης, καμία σελίδα developers/API. Προτεινόμενη λύση:
      **Open-Meteo** (open-meteo.com, άσχετο brand) — δωρεάν, χωρίς
      εγγραφή/API key, αρκετό για όγκο χρήσης εργαστηρίου. Ιδέα: μία
      φορά συντεταγμένες Γαλάτιστας στις Ρυθμίσεις, αυτόματο αίτημα σε
      κάθε καταχώρηση δείγματος (θερμοκρασία/υγρασία/βροχή). Καμία
      σχεδίαση/κώδικας δεν έχει ξεκινήσει.
