...
(() => {
    ...
    App.showModal('🗑 Clean Start — Κλείσιμο Περιόδου', `
      <div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.3);
        ...
        <div style="font-size:13px;color:var(--text-muted);">
          Η διαδικασία θα διαγράψει <strong style="color:var(--text);">${sampleCount} δείγματα</strong>
          και όλες τις εξαρτημένες δοκιμές της τρέχουσας περιόδου
          <strong style="color:var(--text);">${_esc(period?.ce_number || '')}</strong>.
        </div>
      </div>
      <div style="font-size:13px;margin-bottom:8px;"><strong>Πριν τη διαγραφή θα γίνει αυτόματα:</strong></div>
      <div style="font-size:13px;color:var(--text-muted);display:flex;flex-direction:column;gap:6px;">
        <div>✓ Final backup (VACUUM INTO) στον φάκελο της τρέχουσας περιόδου</div>
        <div>✓ Διαγραφή daily backups — παραμένει μόνο το FINAL</div>
        <div>✓ Cloud sync (αν έχει ρυθμιστεί)</div>
        <div>✓ Reset μετρητή δειγμάτων</div>
      </div>
      <div style="margin-top:14px;font-size:13px;color:var(--text-muted);">
        Για να συνεχίσετε πληκτρολογήστε <strong>CLEAN</strong>:
      </div>
  ...
  async function _doCleanStart() {
    ...
    const result = await window.pyBridge?.['clean-start']?.();
    if (result?.ok) {
      App.toast(
        `Clean Start ολοκληρώθηκε — ${result.deleted} δείγματα διαγράφηκαν`,
        'ok'
      );
      await loadCePeriods();
      await loadStorageSettings();
