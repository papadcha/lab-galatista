...
// ============================================================
// Ο διαχειριστής και ο χειριστής μοιράζονται το ίδιο cloud remote —
// ο αυτόματος καθαρισμός παλαιών backups στο remote είναι επικίνδυνος
// αν ενεργοποιηθεί ταυτόχρονα σε δύο εγκαταστάσεις με διαφορετικό
// αριθμό ημερών (η πιο αυστηρή θα "έτρωγε" τα backups που η άλλη
// θεωρεί ακόμα έγκυρα). Λύση: ένα lock file πάνω στο ίδιο το remote
// δηλώνει ποια εγκατάσταση έχει το δικαίωμα να τρέχει αυτόματο
// καθαρισμό· μόνο αυτή περνάει τον έλεγχο.
...
async function _maybeRunAutoRetention(remotePath) {
  const cfg = loadConfig();
