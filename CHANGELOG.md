...
- **Προδιαγραφές κοκκομετρίας ανά υποπερίοδο** — η κοκκομετρία (`tbl_specifications`, όρια περάσματος ανά κόσκινο) παραμένει per-προϊόν μόνο, όχι per-υποπερίοδο. Επιβεβαιώθηκε ότι χρειάζεται τελικά και αυτό, αλλά μεγαλύτερο UI lift (πλέγμα τιμών ανά κόσκινο × προϊόν × υποπερίοδο) — ξεχωριστή εκκρεμότητα, εκτός scope του `tbl_subperiod_specs` (MB/SE/FL) που υλοποιήθηκε.
